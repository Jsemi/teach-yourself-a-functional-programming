object Functions {
    ...
    def filterShortAndTurnToUppercase(names:List[String]) = names.filter(x => x.length>3).map(y => y.toUpperCase)
    ...
    def sumOfLists20(xs: List[List[Int]]): List[Int] = xs.map(x => x.sum).filter(y => y > 20)
